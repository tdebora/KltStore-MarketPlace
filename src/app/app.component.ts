...
export class AppComponent implements OnInit {
            ...
            .then((accountData: any) => {
              ...
              this.web3.getContract()
                .then((contractRes: any) => {
                  if (contractRes) {
                    this.Kltstore = contractRes;
                    this.Kltstore.methods.productCounter()
                      .call()
                      .then(value => {
                        for (let i = 1; i <= value; i++) {
                          const product = this.Kltstore.methods.products(i)
                            .call()
                            .then(products => {
                              this.show = false;
  ...
  private createProducts(name,category, price) {
    this.show = true;
    const etherPrice = this.web3.convertPriceToEther(price);
    this.Kltstore.methods.createProduct(name, category, etherPrice)
  ...
  private purchaseProducts(id, price) {
    this.show = true;
    this.Kltstore.methods.purchaseProduct(id)
